...
void SceneView::updateWorkspace()
{
    ...
    m_program->setAttributeBuffer("position", GL_FLOAT, 0, vertices.size() / 2);
...
void SceneView::resizeGL(int width, int height)
{
    ...
    const Coordinate& max{ m_sceneBounds.max };
    const double bbWidth{ getWidth(m_sceneBounds) };
    const double bbHeight{ getHeight(m_sceneBounds) };

    glViewport(min.x, min.y, bbWidth, bbHeight);

    m_projMatrix.setToIdentity();
    m_projMatrix.ortho(min.x, min.x + bbWidth, min.y + bbHeight, min.y, -1.0F, 1.0F);
}
